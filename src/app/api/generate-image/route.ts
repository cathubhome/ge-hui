  ...
  try {
    ...
    if (!getCpaApiKey()) {
      return NextResponse.json(
        { error: "还没配置绘图服务密钥，请先在服务器环境变量里设置 CPA_API_KEY 哦～" },
        { status: 400 },
      );
    }
...
async function generateWithCpa(plan: ScenePlan, model: string): Promise<string> {
  const panelDesc = plan.panels
    .slice(0, 8)
    .map((p, i) => `${i + 1}) label "${p.labelEn}" / "${p.labelZh}", action: ${p.action}`)
    .join("; ");

  const prompt = `Create ONE educational children's picture-book PAGE (not a random art poster).
